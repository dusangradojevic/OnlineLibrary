...
*/
  ...
  public takeBook = (req: express.Request, res: express.Response) => {
    ...
    Book.updateOne({ id: bookId }, { $inc: { numberInStock: -1 } }, (err: any, resp: any) => {
      ...
      RentLength.findOne({ id: 0 }, (err2: any, rentLen: any) => {
        if (err2) {
          res.json({ message: "Error" });
          return;
        }
        ...
        if (user.rentedBooks.length < 3) {
          ...
          if (bookIndex == -1) {
            ...
            if (expiredBook == false) {
              ...
              let len = await RentLength.findOne({ id: 0 });
              ...
              await User.updateOne({ id: userId }, { $push: { rentedBooks: bookId, rentedBooksDateStart: today, rentedBooksDateEnd: returnDay, rentedBooksExtended: false,reservedBooksAvailable: bookId } });
            }
          }
        }
      }
